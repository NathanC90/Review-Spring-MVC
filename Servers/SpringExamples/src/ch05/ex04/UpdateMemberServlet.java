...
@WebServlet("/ch05/ex04/updateMember.do")
public class UpdateMemberServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		MemberDAO dao = (MemberDAO)context.getBean("memberDAOSupport");
		int count = 0 ;
		...
		if (modify.equalsIgnoreCase("DELETE")) {
			...
			if (!errorMsg.isEmpty()) {
				RequestDispatcher rd = request.getRequestDispatcher("ex04/updateMember.jsp");
		...
		response.sendRedirect(request.getContextPath() + "/ch05/ex04/queryAllMembers.do");
	}
}
